...
def sd_server_fpd1():
    ...
    if os.path.exists("sdxl_config_i8.json"):
        os.remove("sdxl_config_i8.json")
...
def test_sd_server(sd_server_fpd1):
    ...
    ref_path = os.path.join(ref_path, "refs/snow_cat.png")
    if not os.path.exists(ref_path):
        assert 0, f"Reference image does not exist at {ref_path}"
    ref_image = Image.open(ref_path)
    ref_image_numpy = numpy.array(ref_image)
    gen_image_numpy = numpy.array(imgs[0])
    assert (ref_image_numpy == gen_image_numpy).all()
